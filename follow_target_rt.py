...
class StreamingExample(threading.Thread):
    ...
    def yuv_frame_cb(self, yuv_frame):
        ...
        for cnt in contours:
            ...
            # saving center of bounding box of this frame
            xc = x+w/2
            yc = y+h/2
        ...
        # if escape key pressed don't move
        if cv2.waitKey(1) == 27:
            pass
        elif (abs(dx)<1 and abs(dy)<1) and (abs(dx)>0.01 and abs(dy)>0.01):
            print('xcam = %f, ycam = %f' %(x_cam_c,y_cam_c))
            print('xbox = %f, ybox = %f' %(xc,yc))
